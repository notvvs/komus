...
class ParserStateManager:
    ...
    async def _save_to_mongodb(self, state: Dict):
        ...
        try:
            ...
            logger.info("💾 Состояние сохранено в MongoDB")
        except Exception as e:
            logger.error(f"❌ Ошибка сохранения состояния в MongoDB: {e}")
            # Fallback to file
            self._save_to_file(state)
    ...
    async def _load_from_mongodb(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Ошибка загрузки состояния из MongoDB: {e}")
        return None
    ...
    async def _clear_mongodb_state(self):
        ...
        try:
            ...
            logger.error(f"❌ Ошибка удаления состояния из MongoDB: {e}")
